		...
		<div class="col-sm-11">
			<table>
				<tbody>
					<tr>
						<th>생산 지시 코드</th>
						<td><input type="text" id="indicaList" readonly></td>
					</tr>
					<tr>
						<th>공정명</th>
						<td><input type="text" id="prcsDeta" readonly></td>
					</tr>
					<tr>
						<th>불량 분류</th>
						<td><select name="infer" id="infer">
					...
					<tr>
						<th>계획 일자</th>
						<td><input type="date" id="startDt" required> ~ <input
							type="date" id="endDt" required></td>
						<td>
	...
	<script>
		...
		const prcsList = new Grid({
			...
			data : {
				api : {
					readData : {
						url : '${path}/prd/prcsListGrid.do',
						method : 'GET'
					},
					modifyData : {
						url : '${path}/prd/updPrdtInferCode.do',
						method : 'PUT'
					}
				},
		...
		//모달창 출력
		document.getElementById("prcsDeta").addEventListener("click",
				function() {
					callPrcsModal();
				});
		// 공정명 출력
		...
		//지시 출력
		document.getElementById("indicaList").addEventListener("click",
				function() {
					callIndicaModal();
				});
		...
		$("#btnClear").on(
				...
				function() {
					...
					$("#indicaList").val('');
					$('#infer').val('');
					document.getElementById('startDt').value = new Date(year,
							month, day - 7).toISOString().substring(0, 10);
					document.getElementById('endDt').value = new Date()
